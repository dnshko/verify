...
if(isset($_POST["query"]))
{
	...
	$connect = new PDO("mysql:host=localhost; dbname=verify", "root", "");
	...
	if($_POST["query"] != '')
	{
		...
		$query = "
		SELECT id_number, name ,college,training,doc 
		FROM post 
		WHERE id_number LIKE :id_number 
		OR name LIKE :name 
		OR college LIKE :college 
		OR training LIKE :training 
		ORDER BY id DESC
		";
		...
		{
			$data[] = array(
				'id_number'			=>	str_ireplace($replace_array_1, $replace_array_2, $row["id_number"]),
				'name'		=>	str_ireplace($replace_array_1, $replace_array_2, $row["name"]),
				'college'	=>	str_ireplace($replace_array_1, $replace_array_2, $row["college"]),
				'training'		=>	str_ireplace($replace_array_1, $replace_array_2, $row["training"]),
				'doc'		=>	str_ireplace($replace_array_1, $replace_array_2, $row["doc"]),
			);
		}
